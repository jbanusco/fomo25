...
from utils.utils import load_data_pickle, save_data_pickle, parse_dataset_structure, load_json, save_json, load_kfold_split_from_json
...
def main():
    ...
    parser.add_argument("--path_splits", type=str, default="", help="Path to the splits .json")
    parser.add_argument(
    ...
    aug_append = args.augmentation_preset
    if task_type == "segmentation":
        save_dir = os.path.join(args.save_dir, task_name, args.model_name, f"split_{args.split_idx}_aug_{aug_append}_skip")  # We added skip connections
    else:
        save_dir = os.path.join(args.save_dir, task_name, args.model_name, f"split_{args.split_idx}_aug_{aug_append}")
    ...
    try:
        ...
        logging.error(f"Error detecting version: {e}")
        version = 0
    ...
    if os.path.isfile(path_splits):
        # Now, some custome options for our splis parsing
        ...
        print(f"Loading splits from {path_splits} with split method {args.split_method} and param {split_param}")
        splits_config = load_kfold_split_from_json(path_splits, idx=0, param=5, task_prefix=task_prefix, strip_sub=strip_sub)
        print(splits_config)
    else:
        # Create dataset splits
        if args.split_method == "kfold":
    ...
    checkpoint_every_n = ModelCheckpoint(
        ...
        every_n_epochs=10,
        save_last=True,
        save_top_k=-1,  # keep all checkpoints saved every 10 epochs
    )
    ...
    callbacks = [checkpoint_every_n, checkpoint_best]
    ...
    aug_params["mask_ratio"] = 0  #NOTE!! - DISABLE MASKING!
    ...
    data_module = ModYuccaDataModule(
        train_dataset_class=(
            ModYuccaTrainDataset if task_type == "segmentation" else FOMODataset
        ),
    ...
    if run_type == "finetune":
        if os.path.exists(os.path.join(save_dir, f"version_{version}/checkpoints/last.ckpt")):
            print(f"Resuming from checkpoint: {ckpt_path}")            
        else:
            print("Transferring weights for finetuning")
            print(f"Pretrained weights path: {args.pretrained_weights_path}")
            assert args.pretrained_weights_path is not None, "Pretrained weights path not specified"
            ...
            assert num_successful_weights_transferred > 0, "No weights were successfully transferred"
    else:
