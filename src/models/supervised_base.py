...
class BaseSupervisedModel(L.LightningModule):
    ...
    def load_model(self):
        ...
        model_kwargs = {
            ...
            "use_vae": False, # Use deterministic fine-tuning
    ...
    def validation_step(self, batch, _batch_idx):
        ...
        results_dict = self.run_predict(inputs.float())
    ...
    def configure_model_for_finetuning(self):
        ...
        #NOTE: We actually don't use it... this was in case that we wanted to add
        # the self-reconstruction as aux. loss
        for param in self.model.decoder.parameters():
            param.requires_grad = False

        # Freeze also the latent proejctions
        # self.model.conv_mu_shared.requires_grad = False    
        # self.model.conv_logvar_shared.requires_grad = False
        # self.model.conv_mu_modality.requires_grad = False
        # self.model.conv_logvar_modality.requires_grad = False
    ...
    def run_predict(self, inputs):
        ...
        # Get predictions with sliding window
        overlap = 0.5
        with torch.autocast("cuda", enabled=False):  # disables autocast just here
